...
async def get_consultation_history_api(user_id: int):
    """상담 내역 조회"""
    try:
        consultations = await get_consultation_history_service(user_id)
...
async def get_consultation_history_by_id_api(consultation_id: int):
    """상담 내역 조회"""
    try:
        consultation = await get_consultation_history_by_id_service(consultation_id)
...
async def insert_consultation(consultation: ConsultationHistory):
    """상담 내역 추가"""
    try:
        consultation = await insert_consultation_service(consultation)
        if consultation:
...
async def update_consultation(consultation_id: int, consultation: ConsultationHistory):
    """상담 내역 수정"""
    try:
        consultation = await update_consultation_service(consultation_id, consultation)
        if consultation:
            response = JSONResponse(status_code=200, content=consultation)
            return response
        else:
...
async def delete_consultation(consultation_id: int):
    """상담 내역 삭제"""
    try:
        consultation = await delete_consultation_service(consultation_id)
        if consultation:
            response = JSONResponse(status_code=200, content=consultation)
            return response
        else:
...
async def request_consultation(consultation: ConsultationHistory):
    """상담 요청"""
    try:
        consultation = await request_consultation_service(consultation)
        if consultation:
            response = JSONResponse(status_code=200, content=consultation)
            return response
        else:
